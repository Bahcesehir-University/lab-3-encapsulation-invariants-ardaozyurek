...
private:
    ...
    static void validate(const string& pwd) {
        ...
        if (pwd.length()<8){
            throw invalid_argument("Pasword must contain more than 8 digits");
        }
        if (!hasDigit(pwd)){
            throw invalid_argument("Pasword must be at least 1 charcters");
        }
...
public:
    ...
    void change(const string& oldPassword, const string& newPassword) {
        ...
        if (oldPassword!=password_){
            throw invalid_argument ("Old pasword doesnt mach with new pasword");
        }
